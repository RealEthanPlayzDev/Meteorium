...
export const Command: MeteoriumCommand = {
    InteractionData: new SlashCommandBuilder()
        ...
        .addIntegerOption(option => option.setName("case").setDescription("The case id for the case to be viewed").setRequired(true)),
    async Callback(interaction, client) {
        if (!interaction.member.permissions.has("ViewAuditLog")) return await interaction.editReply({ content: "You do not have permission to view a user's punishment/case." });
        ...
        return await interaction.reply({
            embeds: [
                new MeteoriumEmbedBuilder(undefined, interaction.user)
                    .setTitle("")
                    .setDescription("")
                    .setAuthor({ name: `Case: #${CaseId} | kick | ${Case.TargetUserId}` }) //, iconURL: User.displayAvatarURL({ extension: "png" }) })
                    .addFields(
                        { name: "User", value: `<@${Case.TargetUserId}>` },
                        { name: "Moderator", value: `<@${Case.ModeratorUserId}>` },
