...
export const Command: MeteoriumCommand = {
    InteractionData: new SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName("reason").setDescription("The reason on why the user was warned").setRequired(true))
    ...
    async Callback(interaction, client) {
        ...
        if (!interaction.member.permissions.has("ViewAuditLog")) return await interaction.editReply({ content: "You do not have permission to warn users from this server." });
        ...
        return await interaction.reply({
            embeds: [
                new MeteoriumEmbedBuilder(undefined, interaction.user)
                    .setTitle("")
                    .setDescription("")
                    .setAuthor({ name: `Case: #${CaseResult.CaseId} | warn | ${User.username}`, iconURL: User.displayAvatarURL({ extension: "png" }) })
                    .addFields(
