...
export const Command: MeteoriumCommand = {
    ...
    async Callback(interaction) {
        ...
        const Channel = interaction.options.getChannel("channel", false) ? interaction.options.getChannel("channel", false) : interaction.channel;
        const MessageId = interaction.options.getString("messageid", true);
        const Emoji = interaction.options.getString("emoji", true);

        if (!interaction.member.permissions.has("ManageMessages")) return await interaction.editReply({ content: "You do not have permission to use this command. (Missing ManageMessages permission)" });
        if (!Channel) return await interaction.editReply({ content: "No channel? Please try again later" });
        if (!Channel.isTextBased()) return await interaction.editReply({ content: "Please provide a text-based channel!" });

        let TargetMessage, TargetEmoji;
        try {
            TargetMessage = await Channel.messages.fetch(MessageId);
        } catch(e) { console.error(`Error while getting message: ${e}`) }
        try {
        ...
        } catch(e) { console.error(`Error while getting emoji: ${e}`) }
