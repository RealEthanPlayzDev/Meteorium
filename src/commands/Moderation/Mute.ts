...
export const Command: MeteoriumCommand = {
    InteractionData: new SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName("reason").setDescription("The reason on why the user was muted").setRequired(true))
        .addIntegerOption(option => option.setName("duration").setDescription("The duration of the mute").setRequired(true))
    ...
    async Callback(interaction) {
        if (!interaction.member.permissions.has("MuteMembers")) return await interaction.editReply({ content: "You do not have permission to mute users from this server." });
